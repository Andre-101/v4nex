...
class AdminAuditEvent(Base):
    ...
    bridge_id: Mapped[str | None] = mapped_column(ForeignKey("bridges.id"), nullable=True, index=True)
    action: Mapped[str] = mapped_column(String(80), nullable=False)
    message: Mapped[str] = mapped_column(String(500), nullable=False)
    event_metadata: Mapped[dict] = mapped_column("metadata", JSON, default=dict, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=utc_now, nullable=False)

    actor_user = relationship("User", foreign_keys=[actor_user_id], back_populates="admin_audit_events")
